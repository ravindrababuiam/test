        ...
        try {
            if (-not $WhatIf) {
                ...
                if ($graphToken) {
                    ...
                    $intuneUrl = "https://graph.microsoft.us/v1.0/deviceManagement/managedDevices/$intuneDeviceId"
        ...
        try {
            if (-not $WhatIf) {
                ...
                if ($graphToken) {
                    ...
                    $entraUrl = "https://graph.microsoft.us/v1.0/devices/$entraDeviceId"
...
# Extract subscription identifier from session hostname (7th and 8th characters)
$subscriptionIdentifier = $null
if ($SessionHostName.Length -ge 8) {
    $subscriptionIdentifier = $SessionHostName.Substring(6, 2)
...
if (-not $ResourceGroupName) {
    ...
    if (-not $SkipGraphAPI) {
        try {
            ...
            $graphTokenUrl = "https://login.microsoftonline.us/$TenantId/oauth2/v2.0/token"
            ...
            if ($graphToken) {
                ...
                $deviceUrl = "https://graph.microsoft.us/v1.0/devices?`$filter=$deviceFilter"
                ...
                try {
                    ...
                    $intuneUrl = "https://graph.microsoft.us/v1.0/deviceManagement/managedDevices?`$filter=$intuneFilter"
